...
public class UnitTest {
	...
	public void testFightWhenThisCaloriesIsBetter() {
		...
		uB.challenge(uA);
		assertEquals(500, uA.getCalories());
		assertEquals(499 - 2 * Globals.CALORIE_LOSS_PER_CHALLENGE,
				uB.getCalories());
	}
	...
	public void testFightWhenOthersCaloriesIsBetter() {
		...
		uB.challenge(uA);
		assertEquals(800 - 2 * Globals.CALORIE_LOSS_PER_CHALLENGE,
				uA.getCalories());
		assertEquals(900, uB.getCalories());
	}
	 ...
	 * 
	 * Current plan is to have one take another bandana when one has twice as
	 * many calories (or the other has only half)
	 */
	@Test
	public void testOneUnitGetsAnothersFlag() {
		...
		assertEquals(intialCaloriesOfA, uA.getCalories());
		assertEquals(
				intialCaloriesOfB - 2 * Globals.CALORIE_LOSS_PER_CHALLENGE,
				uB.getCalories());
	}
	 ...
	 * the Unit's profile adds an Award when the Unit has won (taken a bandana)
	 * 10 times
	 */
	@Test
